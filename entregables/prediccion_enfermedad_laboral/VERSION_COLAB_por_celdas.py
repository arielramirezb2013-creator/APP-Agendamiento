# =============================================================================
# CELDA 1 — INSTALACION DE DEPENDENCIAS (ejecutar primero, solo una vez)
# =============================================================================
!pip -q install pandas numpy matplotlib scikit-learn openpyxl pyarrow joblib
# Opcionales (mejoran el modelo si estan disponibles):
# !pip -q install lightgbm shap


# =============================================================================
# CELDA 2 — ENCABEZADO, IMPORTS
# =============================================================================
# -*- coding: utf-8 -*-
"""
PIPELINE CONSOLIDADO V4.0 - MODELO PREDICTIVO DE ENFERMEDAD LABORAL
===============================================================================
...
Qué predice esta versión (V4.0)
-------------------------------
Además del score de riesgo por trabajador, la V4 proyecta, para cada horizonte configurado
(12, 24 y 60 meses por defecto):
  1. Casos esperados de enfermedad laboral en la población activa, con intervalo del 95%.
  2. Qué PATOLOGÍAS son más probables (modelos específicos por patología cuando hay masa
     crítica; descomposición score global × mezcla histórica cuando no la hay).
  3. Qué CARGOS y CENTROS DE TRABAJO (sede/ciudad) concentran esos casos esperados.
Todo se exporta en tablas CSV y en un informe markdown (informe_predictivo.md) pensado para
leerse directamente o pegarse en un asistente de IA para su análisis.

Las proyecciones son valores esperados condicionales: asumen que los patrones históricos de
la organización se mantienen y llevan una bandera explícita de confiabilidad por horizonte.
No constituyen diagnóstico, calificación de origen ni determinación de PCL.

Versión: 4.0 predictiva
Fecha documental: 2026-08-13
"""
...
class ProjectConfig:
    ...
    HORIZONS_MONTHS: Tuple[int, ...] = (12, 24, 60)
    ...
    MIN_TRAIN_POSITIVES_AFTER_PURGE: int = 10
    ...
    # Prediccion por patologia y proyecciones de casos esperados
    ENABLE_PATHOLOGY_MODELS: bool = True
    PATHOLOGY_MIN_POSITIVES: int = 25     # minimo de positivos para modelo propio por patologia
    PATHOLOGY_MIX_SMOOTHING: float = 5.0  # encogimiento de la mezcla por cargo hacia la global
    GENERATE_PREDICTIVE_REPORT: bool = True
...
CANONICAL_SYNONYMS: Dict[str, Tuple[str, ...]] = {
    ...
    "centro": (
        ...
        "ciudad", "municipio", "regional", "sucursal", "planta", "proyecto",
    ),
    "sex": (
...
MSK_KEYWORDS: Tuple[str, ...] = (
    ...
    "manguito", "codo", "epicond", "muñeca", "muneca", "mano", "tendin", "tendon",
    "tenosinov", "tunel", "túnel", "disco", "columna", "miembro superior", "miembro inferior",
)
...
# Taxonomia de patologias para la prediccion diferenciada. El grupo se asigna primero por
# prefijo CIE-10 (sin punto) y, en su defecto, por palabras clave del texto de diagnostico.
# El orden importa: el primer grupo que coincide gana; "otras" es el residual. La taxonomia
# es editable por empresa: agregue o divida grupos segun su perfil epidemiologico real.
PATHOLOGY_GROUPS: Dict[str, Dict[str, Any]] = {
    "osteomuscular": {
        "label": "Trastornos musculoesqueleticos (DME)",
        "cie10_prefixes": ("M", "G56", "G57"),
        "keywords": MSK_KEYWORDS,
    },
    "auditiva": {
        "label": "Hipoacusia y trastornos auditivos",
        "cie10_prefixes": ("H83", "H90", "H91", "H93"),
        "keywords": ("hipoacusia", "audit", "sordera", "trauma acustico", "acufeno", "tinnitus"),
    },
    "respiratoria": {
        "label": "Enfermedades respiratorias ocupacionales",
        "cie10_prefixes": ("J",),
        "keywords": ("asma", "neumoconiosis", "silicosis", "asbestosis", "epoc", "rinitis", "respirat", "pulmonar", "alveolitis"),
    },
    "mental": {
        "label": "Trastornos mentales y del comportamiento",
        "cie10_prefixes": ("F",),
        "keywords": ("estres", "ansiedad", "depresi", "salud mental", "psicosocial", "burnout", "agotamiento", "adaptativo"),
    },
    "dermatologica": {
        "label": "Enfermedades de la piel",
        "cie10_prefixes": ("L",),
        "keywords": ("dermatitis", "dermat", "eczema", "urticaria", "piel"),
    },
    "otras": {
...
# =============================================================================
# CELDA 4 — 2. UTILIDADES GENERALES, LOGS Y PRIVACIDAD
# =============================================================================
# %%
# =============================================================================
...
def _contains_keyword_any(series: pd.Series, keywords: Sequence[str]) -> pd.Series:
    """Como value_contains_any, pero exige que la coincidencia empiece en limite de palabra:
    la raiz "tendin" matchea "tendinitis" pero una raiz corta ya no puede activarse dentro de
    otra palabra (el clasico "tend" dentro de "atendido"). Se usa para clasificar patologias y
    features MSK, donde el texto administrativo ("atendido por...") es casi universal."""
    normalized = series.fillna("").astype(str).map(normalize_text)
    pattern = "|".join(
        r"\b" + re.escape(normalize_text(k)) for k in keywords if normalize_text(k)
    )
    if not pattern:
        return pd.Series(False, index=series.index)
    return normalized.str.contains(pattern, regex=True, na=False)
...
# =============================================================================
# CELDA 5 — 3. INVENTARIO, LECTURA Y MAPEADO DE FUENTES
# =============================================================================
# %%
# =============================================================================
...
# =============================================================================
# CELDA 6 — 4. CALIDAD DE DATOS Y PLANTILLAS
# =============================================================================
# %%
# =============================================================================
...
# =============================================================================
# CELDA 7 — 5. UNIDAD ANALITICA, ROSTER Y PANEL TEMPORAL
# =============================================================================
# %%
# =============================================================================
...
def collect_roster(data: Dict[str, pd.DataFrame], unit: str) -> pd.DataFrame:
    ...
    if "termination_date" in roster.columns and "hire_date" in roster_raw.columns:
        ...
        # Desempate determinista: orden estable por hire_date y, dentro del mismo ciclo
        # (misma hire_date replicada por varias fuentes), na_position="first" deja de ultima
        # la fila que SI trae termination_date, evitando que una fuente sin esa columna
        # sobreescriba con NaT el retiro real del trabajador.
        latest_stint_termination = (
            roster_raw[roster_raw["hire_date"].notna()]
            .sort_values(["hire_date", "termination_date"], kind="stable", na_position="first")
...
# =============================================================================
# CELDA 8 — 6. FEATURES DE EVENTOS, EXPOSICION Y VENTANAS HISTORICAS
# =============================================================================
# %%
# =============================================================================
...
def diagnosis_is_msk(df: pd.DataFrame) -> pd.Series:
    ...
    for col in ["diagnosis", "body_part", "risk_factor", "event_type"]:
        if col in df.columns:
            mask |= _contains_keyword_any(df[col], MSK_KEYWORDS)
    return mask
 
 
def classify_pathology_group(df: pd.DataFrame) -> pd.Series:
    """Asigna a cada registro un grupo de PATHOLOGY_GROUPS por CIE-10 y texto de diagnostico.

    El primer grupo que coincide (en el orden de la taxonomia) gana; los registros sin
    coincidencia quedan en "otras". El CIE-10 se normaliza sin punto (H90.3 -> H903)."""
    if df.empty:
        return pd.Series(dtype="object")
    result = pd.Series("otras", index=df.index, dtype="object")
    assigned = pd.Series(False, index=df.index)

    if "cie10" in df.columns:
        cie = (
            df["cie10"].fillna("").astype(str).str.upper()
            .str.replace(".", "", regex=False).str.strip()
        )
    else:
        cie = pd.Series("", index=df.index)

    text = pd.Series("", index=df.index, dtype="object")
    for col in ["diagnosis", "status", "event_type", "recommendation"]:
        if col in df.columns:
            text = text + " " + df[col].fillna("").astype(str)

    # Pasada 1: SOLO prefijos CIE-10. Un codigo explicito (F43, J45, L23, H90...) siempre
    # domina sobre el texto libre: sin esta separacion, una palabra debil del primer grupo
    # (p. ej. una raiz MSK presente en texto administrativo) anularia el CIE-10 de grupos
    # posteriores e inflaria silenciosamente la primera patologia de la taxonomia.
    for group, spec in PATHOLOGY_GROUPS.items():
        if group == "otras" or not spec["cie10_prefixes"]:
            continue
        mask = pd.Series(False, index=df.index)
        for prefix in spec["cie10_prefixes"]:
            mask |= cie.str.startswith(prefix)
        result[mask & ~assigned] = group
        assigned |= mask

    # Pasada 2: keywords (con limite de palabra inicial) solo para lo aun no asignado.
    for group, spec in PATHOLOGY_GROUPS.items():
        if group == "otras" or not spec["keywords"]:
            continue
        mask = _contains_keyword_any(text, spec["keywords"])
        result[mask & ~assigned] = group
        assigned |= mask
    return result
...
# =============================================================================
# CELDA 9 — 7. TARGET DE ENFERMEDAD LABORAL Y CONTROL DE SESGOS
# =============================================================================
# %%
# =============================================================================
...
def _future_targets_for_events(
    out: pd.DataFrame,
    events: pd.DataFrame,
    key: str,
    cfg: ProjectConfig,
    last_observed_by_entity: Dict[Any, int],
    apply_first_event_exclusion: bool,
    coverage_cap_int: Optional[int],
) -> Tuple[Dict[int, np.ndarray], Dict[int, np.ndarray]]:
    """Construye targets y elegibilidad por horizonte para UN conjunto de eventos dado.

    La censura usa el fin de seguimiento propio de cada entidad. La exclusion tras el primer
    evento (EXCLUDE_AFTER_FIRST_CONFIRMED_EL) aplica sobre ESTE conjunto de eventos: para el
    target de una patologia especifica, una persona sigue en riesgo de esa patologia aunque
    ya tenga confirmada una patologia distinta."""
    ...
    for idx, (entity, current_int) in enumerate(zip(out[key], out["period_int"])):
        ...
        if apply_first_event_exclusion and entity in first_event and current_int >= first_event[entity]:
            continue

        entity_last = last_observed_by_entity.get(entity)
        if entity_last is None or pd.isna(entity_last):
            continue
        entity_last_int = int(entity_last)
        if coverage_cap_int is not None:
            # C6: el seguimiento del TARGET termina donde termina la cobertura del extracto de
            # casos EL, aunque otras fuentes (nomina, ausentismo) lleguen mas lejos: sin esto,
            # los meses posteriores al corte del archivo EL se etiquetarian como negativos.
            entity_last_int = min(entity_last_int, coverage_cap_int)
        ...
        for horizon in cfg.HORIZONS_MONTHS:
            ...
            # usando el fin de seguimiento propio de la entidad (retiro incluido).
            if current_int + horizon > entity_last_int:
                continue
            eligibility[horizon][idx] = 1
            if next_event is not None and 1 <= next_event - current_int <= horizon:
...
def add_future_targets(
    ...
    """Construye el target global de EL futura y, ademas, un target por grupo de patologia
    (target_el_{h}m_pat_{grupo}) para todo grupo con al menos un evento confirmado."""
    out = panel.copy()
    rules: List[Dict[str, Any]] = []

    def empty_targets(message: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        for horizon in cfg.HORIZONS_MONTHS:
            out[f"target_el_{horizon}m"] = 0
            out[f"target_eligible_{horizon}m"] = 0
        out["target_eligible"] = 0
        log.add("target", "WARN", message)
        return out, pd.DataFrame(rules)

    if el_df.empty or key not in el_df.columns or "periodo" not in el_df.columns:
        return empty_targets("No hay eventos EL utilizables; los targets quedan sin elegibilidad y no se forzará entrenamiento.")
    ...
    if events.empty:
        return empty_targets("No se identificaron eventos target bajo las reglas configuradas.")

    events["event_int"] = month_int(events["periodo"])
    events["pathology_group"] = classify_pathology_group(events)
    last_observed_by_entity = out.groupby(key)["period_int"].max().to_dict()

    # C6: cobertura temporal de la fuente que define el target (todo el extracto EL, no solo
    # los positivos): censura el fin de seguimiento efectivo de todas las entidades.
    _coverage = month_int(el_df["periodo"]).dropna()
    coverage_cap_int = int(_coverage.max()) if not _coverage.empty else None
    # C8: la semantica "riesgo de PRIMER evento" solo aplica a personas. En unidades agregadas
    # (cargo_mes/centro_mes) el primer caso historico de un trabajador no puede censurar a toda
    # la unidad: el target agregado es "algun evento en la ventana futura".
    apply_first_event_exclusion = bool(cfg.EXCLUDE_AFTER_FIRST_CONFIRMED_EL and key == "person_id")

    event_sets: List[Tuple[str, str, pd.DataFrame]] = [("global", "", events)]
    if cfg.ENABLE_PATHOLOGY_MODELS:
        for group in events["pathology_group"].dropna().unique():
            group_events = events[events["pathology_group"].eq(group)]
            if not group_events.empty:
                event_sets.append((str(group), f"_pat_{group}", group_events))

    out["target_eligible"] = 0
    for scope, suffix, scoped_events in event_sets:
        targets, eligibility = _future_targets_for_events(
            out, scoped_events, key, cfg, last_observed_by_entity,
            apply_first_event_exclusion, coverage_cap_int,
        )
        for horizon in cfg.HORIZONS_MONTHS:
            horizon = int(horizon)
            out[f"target_el_{horizon}m{suffix}"] = targets[horizon]
            out[f"target_eligible_{horizon}m{suffix}"] = eligibility[horizon]
            if scope == "global":
            ...
            rules.append({
                ...
                "censoring_rule": "Solo filas con seguimiento completo hasta el horizonte (fin de seguimiento propio de cada entidad, acotado por la cobertura de la fuente EL)",
                "first_event_exclusion": apply_first_event_exclusion,
                "target_source_last_period": str(el_df["periodo"].max())[:7],
    ...
    pathology_scopes = [s for s, _, _ in event_sets if s != "global"]
    if pathology_scopes:
        log.add("target", "OK", f"Targets por patología creados para: {', '.join(sorted(pathology_scopes))}")
    return out, pd.DataFrame(rules)



# =============================================================================
# CELDA 10 — 8. PREPARACION DEL MODELO, FUGA, MULTICOLINEALIDAD Y SPLIT TEMPORAL
# =============================================================================
# %%
# =============================================================================
...
def _purge_overlapping_train_rows(train: pd.DataFrame, cutoff: pd.Timestamp, horizon: int) -> pd.DataFrame:
    """Purga anti-fuga: descarta del entrenamiento las filas cuya ventana de outcome
    (periodo, periodo + horizonte] cruza el corte temporal. Sin esta purga, los labels de los
    ultimos meses de entrenamiento se construirian con eventos ocurridos DENTRO del periodo de
    prueba, y el mismo evento fisico generaria positivos simultaneos en train y test."""
    cutoff_int = cutoff.year * 12 + cutoff.month
    return train[train["period_int"] + horizon < cutoff_int].copy()
...
def temporal_train_test_split(
    ...
    for size in candidate_sizes:
        ...
        train = _purge_overlapping_train_rows(work[work["periodo"] < cutoff], cutoff, horizon)
        test = work[work["periodo"] >= cutoff].copy()
        if train[target_col].nunique() < 2 or test[target_col].nunique() < 2:
            continue
        if int(train[target_col].sum()) < cfg.MIN_TRAIN_POSITIVES_AFTER_PURGE:
            continue
...
def carve_validation_slice(
    ...
    for val_size in range(min(cfg.VALIDATION_MONTHS, max_val_periods), 0, -1):
        ...
        actual_train = _purge_overlapping_train_rows(train_df[train_df["periodo"] < cutoff], cutoff, horizon)
        validation = train_df[train_df["periodo"] >= cutoff].copy()
        if actual_train[target_col].nunique() < 2 or validation[target_col].nunique() < 2:
            continue
        if int(actual_train[target_col].sum()) < cfg.MIN_VALIDATION_POSITIVES:
            continue
...
# =============================================================================
# CELDA 11 — 9. VALIDACION, UMBRAL Y METRICAS
# =============================================================================
# %%
# =============================================================================
...
# =============================================================================
# CELDA 12 — 10. ENTRENAMIENTO POR HORIZONTE Y FALLBACK TRANSPARENTE
# =============================================================================
# %%
# =============================================================================
...
def train_one_horizon(
    ...
    # tag distingue variantes del mismo horizonte (p. ej. "_pat_osteomuscular") en archivos y logs.
    target_col = target_col or f"target_el_{horizon}m"
    eligibility_col = eligibility_col or f"target_eligible_{horizon}m"
    ...
    if positives < cfg.MIN_POSITIVES_TO_TRAIN or negatives < cfg.MIN_NEGATIVES_TO_TRAIN:
        ...
        log.add("modelo", "WARN", f"Horizonte {horizon}m{tag} no entrenado: {result['reason']}")
        return result

    try:
        train_df, test_df, cutoff, split_method = temporal_train_test_split(eligible, target_col, eligibility_col, cfg, int(horizon))
    except Exception as exc:
    ...
    _train_pos = int(train_df[target_col].sum())
    if _train_pos < cfg.MIN_TRAIN_POSITIVES_AFTER_PURGE:
        result["reason"] = (
            f"Masa critica insuficiente tras la purga anti-fuga: {_train_pos} positivos de "
            f"entrenamiento (< {cfg.MIN_TRAIN_POSITIVES_AFTER_PURGE})."
        )
        log.add("modelo", "WARN", f"Horizonte {horizon}m{tag} no entrenado: {result['reason']}")
        return result
    ...
    if split_method != "criterio_completo":
        log.add(
            "modelo", "WARN",
            f"Horizonte {horizon}m{tag}: el split temporal uso el respaldo de ultimo tercio (test con "
    ...
    actual_train_df, validation_df = carve_validation_slice(train_df, target_col, cfg, int(horizon))
    has_validation = validation_df is not None
    if not has_validation:
        log.add(
            "modelo", "WARN",
            f"Horizonte {horizon}m{tag}: no fue posible separar una validacion temporal interna "
    ...
    # C3: el candidato calibrado se ajusta primero sobre actual_train (menos positivos que el
    # train completo); dimensionar su cv interno con el train completo puede hacerlo fallar.
    _cal_positives = int(actual_train_df[target_col].sum()) if has_validation else int(train_df[target_col].sum())
    calibration_cv = max(2, min(3, _cal_positives))
    ...
    for name, estimator in candidates.items():
        try:
            cv_metrics = cross_validate_pipeline(
                ...
                log=log, context=f"{horizon}m{tag}/{name}",
            )
            ...
            if has_validation:
                # Un fallo aqui (p. ej. calibracion con muy pocos positivos en actual_train)
                # degrada al umbral por defecto SIN descartar el ajuste final del candidato.
                try:
                    ...
                    log.add(
                        "modelo", "WARN",
                        f"{horizon}m{tag}/{name}: la validacion interna fallo ({exc}); "
                        "se usa el umbral por defecto y la seleccion dependera de la CV agrupada.",
                    )
            ...
            log.add("modelo", "ERROR", f"{horizon}m{tag} / {name}: {exc}")
    ...
    if cfg.PERSIST_MODELS:
        joblib.dump(best_model, dirs["models"] / f"modelo_{horizon}m{tag}_{best_name}.joblib")

    save_df(metrics_df, dirs["tables"] / f"metricas_modelos_{horizon}m{tag}.csv")
    save_df(feature_trace, dirs["tables"] / f"trazabilidad_features_{horizon}m{tag}.csv")
    if not multicol.empty:
        save_df(multicol, dirs["tables"] / f"multicolinealidad_{horizon}m{tag}.csv")
    if not threshold_tables[best_name].empty:
        save_df(threshold_tables[best_name], dirs["tables"] / f"umbrales_{horizon}m{tag}_{best_name}.csv")
    ...
    log.add(
        "modelo", "OK",
        f"Horizonte {horizon}m{tag}: {best_name}; PR-AUC={best_row['pr_auc']:.3f}; "
...
def empirical_horizon_factors(
    ...
    # CDF empirica con suavizado de Laplace: evita factores exactamente 0 (que anularian o
    # saturarian el cociente entre horizontes) cuando ninguna latencia observada cae bajo h.
    n_pairs = len(matched)
    factors = {
        int(h): float((int((matched["lag_months"] <= h).sum()) + 1) / (n_pairs + 2))
        for h in horizons
    }
...
# =============================================================================
# CELDA 13 — 11. SCORING ACTUAL, PRIORIZACION Y AGREGACIONES
# =============================================================================
# %%
# =============================================================================
...
def _anchor_alerts(
    scores: pd.Series,
    threshold: float,
    modeled_mask: pd.Series,
    cfg: ProjectConfig,
) -> Tuple[pd.Series, float, bool]:
    """Aplica el umbral operativo y, si la tasa de alerta resultante en la poblacion modelada
    excede materialmente el maximo configurado (DECISION_MAX_ALERT_RATE), re-ancla el umbral
    al cuantil correspondiente de la poblacion actual.

    El umbral se eligio sobre la distribucion de scores del periodo de validacion; cuando esa
    distribucion difiere de la del snapshot actual, un umbral 'optimo' historico puede alertar
    a casi toda la plantilla, volviendo la alerta inoperante. El re-anclaje preserva la
    restriccion operativa declarada y queda registrado en la columna de metodo."""
    scores = pd.to_numeric(scores, errors="coerce")
    alerts = (scores >= threshold).astype(int)
    base = alerts[modeled_mask] if modeled_mask is not None and modeled_mask.any() else alerts
    rate = float(base.mean()) if len(base) else 0.0
    if rate > 1.5 * cfg.DECISION_MAX_ALERT_RATE:
        ref = scores[modeled_mask] if modeled_mask is not None and modeled_mask.any() else scores
        new_threshold = float(np.nanquantile(ref, 1 - cfg.DECISION_MAX_ALERT_RATE))
        return (scores >= new_threshold).astype(int), new_threshold, True
    return alerts, float(threshold), False
...
def score_current_population(
    ...
    if lag and len(all_periods) > lag:
        latest_period = pd.Timestamp(all_periods[-(lag + 1)])
    else:
        latest_period = pd.Timestamp(all_periods[-1]) if all_periods else pd.Timestamp(analytic_df["periodo"].max())
    current = analytic_df[analytic_df["periodo"].eq(latest_period)].copy()

    # C5: las entidades con EL confirmada previa salen del universo modelado (el modelo estima
    # riesgo de PRIMER evento); se marcan y se apartan del ranking y de los conteos esperados.
    current["el_confirmado_previo"] = 0
    _el_source = data.get("casos_el", pd.DataFrame())
    if not _el_source.empty and key in _el_source.columns and "periodo" in _el_source.columns:
        _cls = classify_el_records(_el_source, cfg)
        _confirmed_first = (
            _cls[_cls["el_confirmed"].eq(1) & _cls[key].notna() & _cls["periodo"].notna()]
            .groupby(key)["periodo"].min()
        )
        _prior = set(_confirmed_first[_confirmed_first <= latest_period].index)
        if _prior:
            current["el_confirmado_previo"] = current[key].isin(_prior).astype(int)
    ...
    for horizon in cfg.HORIZONS_MONTHS:
        ...
        if meta.get("status") == "trained":
            ...
            _modeled_mask = current["el_confirmado_previo"].eq(0)
            _alerts, _thr, _reanchored = _anchor_alerts(
                current[score_col], float(meta["threshold"]), _modeled_mask, cfg
            )
            current[alert_col] = _alerts
            current[method_col] = meta["calibration_status"] + (
                f"; umbral_reanclado_a_{_thr:.4f}_por_tasa_de_alerta" if _reanchored else ""
            )
            if _reanchored:
                log.add(
                    "scoring", "WARN",
                    f"Horizonte {horizon}m: el umbral de validacion ({float(meta['threshold']):.4f}) "
                    f"alertaba por encima del maximo operativo; re-anclado a {_thr:.4f} "
                    f"(tasa objetivo {cfg.DECISION_MAX_ALERT_RATE:.0%}).",
                )
            ...
            # Cociente de CDFs: escala el score del horizonte base hacia el horizonte
            # objetivo. Un factor absoluto (siempre <= 1) haria que un horizonte MAS largo
            # puntuara por debajo del corto, violando la monotonicidad de las ventanas.
            factor = factors.get(int(horizon), 1.0) / max(factors.get(int(nearest), 1.0), 1e-9)
            # Tope de seguridad: un cociente empirico extremo no debe saturar los scores.
            factor = float(min(factor, 5.0))
            ...
            _modeled_mask = current["el_confirmado_previo"].eq(0)
            _alerts, _thr, _reanchored = _anchor_alerts(
                pd.Series(base, index=current.index), float(nearest_meta["threshold"]), _modeled_mask, cfg
            )
            current[alert_col] = _alerts
    ...
    # Percentil homogeneo: cada horizonte se convierte a percentil dentro de su propia escala
    # (las probabilidades de ventanas largas son naturalmente mayores y dominarian un maximo
    # crudo) y se toma el maximo. El ranking solo incluye poblacion modelada (sin EL previa).
    modeled = current["el_confirmado_previo"].eq(0)
    _pct_frames = [
        pd.to_numeric(current[c], errors="coerce").where(modeled).rank(pct=True, method="average")
        for c in score_cols
    ]
    current["score_percentile"] = pd.concat(_pct_frames, axis=1).max(axis=1)
    ...
    current["prioridad_integrada"] = np.select(
        [~modeled, high | (current["alerta_any"].eq(1) & medium), medium | current["alerta_any"].eq(1)],
        ["Caso_existente_seguimiento", "Alta", "Media"],
        default="Baja",
    )
...
def aggregate_priority(scored: pd.DataFrame, dimension: str, min_n: int = 30) -> pd.DataFrame:
    ...
    if "el_confirmado_previo" in work.columns:
        work = work[work["el_confirmado_previo"].eq(0)]
    if work.empty:
        return pd.DataFrame()
...
# =============================================================================
# CELDA 14 — 11B. PROYECCIONES PREDICTIVAS: PATOLOGIAS, CARGOS Y CENTROS POR HORIZONTE
# =============================================================================
# %%
# =============================================================================
# 11B. PROYECCIONES PREDICTIVAS: PATOLOGIAS, CARGOS Y CENTROS POR HORIZONTE
# =============================================================================

def compute_pathology_mix(
    el_events: pd.DataFrame,
    roster: pd.DataFrame,
    key: str,
    cfg: ProjectConfig,
) -> Tuple[pd.Series, Optional[pd.DataFrame]]:
    """Mezcla historica de patologias: global y por cargo (con encogimiento hacia la global).

    La mezcla por cargo solo se calcula si el cargo puede enlazarse a los eventos (viene en la
    fuente EL o se resuelve via roster). Con pocos eventos por cargo, el encogimiento
    (PATHOLOGY_MIX_SMOOTHING) hace que domine la mezcla global, evitando proporciones
    inestables basadas en 1-2 casos."""
    groups = list(PATHOLOGY_GROUPS)
    if el_events.empty or "pathology_group" not in el_events.columns:
        uniform = pd.Series(1.0 / len(groups), index=groups)
        return uniform, None

    counts = el_events["pathology_group"].value_counts()
    global_mix = counts.reindex(groups).fillna(0.0).astype(float)
    total = float(global_mix.sum())
    if total <= 0:
        global_mix = pd.Series(1.0 / len(groups), index=groups)
    else:
        global_mix = global_mix / total

    events = el_events.copy()
    needs_cargo = "cargo" not in events.columns or events["cargo"].isna().all()
    if needs_cargo and key == "cargo":
        # La llave YA es el cargo; si el extracto EL ni siquiera trae esa columna no hay
        # forma de resolverla (y un merge roster[["cargo","cargo"]] duplicaria la etiqueta
        # y abortaria el pipeline): se usa solo la mezcla global.
        return global_mix, None
    if needs_cargo and (
        not roster.empty and key != "cargo" and key in roster.columns
        and "cargo" in roster.columns and key in events.columns
    ):
        events = events.merge(
            roster[[key, "cargo"]].drop_duplicates(subset=[key]), on=key, how="left", suffixes=("", "_roster"),
        )
        if "cargo_roster" in events.columns:
            events["cargo"] = events["cargo"].fillna(events["cargo_roster"]) if "cargo" in events.columns else events["cargo_roster"]

    if "cargo" not in events.columns or events["cargo"].isna().all():
        return global_mix, None

    alpha = float(cfg.PATHOLOGY_MIX_SMOOTHING)
    rows: List[Dict[str, Any]] = []
    for cargo, grp in events[events["cargo"].notna()].groupby("cargo"):
        n_c = len(grp)
        cnt = grp["pathology_group"].value_counts()
        for g in groups:
            n_gc = float(cnt.get(g, 0))
            rows.append({
                "cargo": cargo,
                "pathology_group": g,
                "mix": (n_gc + alpha * float(global_mix[g])) / (n_c + alpha),
            })
    mix_by_cargo = pd.DataFrame(rows).pivot(index="cargo", columns="pathology_group", values="mix")
    return global_mix, mix_by_cargo


def _expected_with_ci(p: pd.Series) -> Tuple[float, float, float]:
    """Casos esperados = suma de probabilidades individuales; IC 95% por aproximacion normal
    de la Poisson-binomial (varianza = suma p*(1-p))."""
    e = float(p.sum())
    sd = float(np.sqrt((p * (1.0 - p)).clip(lower=0).sum()))
    return e, max(0.0, e - 1.96 * sd), e + 1.96 * sd


def project_predictions(
    scored: pd.DataFrame,
    horizon_models: Dict[int, Dict[str, Any]],
    pathology_models: Dict[str, Dict[int, Dict[str, Any]]],
    el_events: pd.DataFrame,
    roster: pd.DataFrame,
    key: str,
    cfg: ProjectConfig,
    ...
    """Convierte los scores por trabajador en proyecciones accionables por horizonte:

    - casos esperados totales (IC 95%),
    - casos esperados por PATOLOGIA (modelo especifico si existe; si no, descomposicion
      score global x mezcla historica de patologias, suavizada por cargo),
    - casos esperados y patologias dominantes por CARGO y por CENTRO de trabajo,
    - patologia mas probable por trabajador (columna en la tabla de scoring).

    Cada fila lleva una bandera de confiabilidad: cuando el horizonte se puntuo con el
    fallback de percentiles (sin modelo entrenado), NO se proyectan conteos (solo ranking)."""
    global_mix, mix_by_cargo = compute_pathology_mix(el_events, roster, key, cfg)
    groups = list(PATHOLOGY_GROUPS)
    out = scored.copy()
    ...
    centro_rows: List[Dict[str, Any]] = []

    for horizon in cfg.HORIZONS_MONTHS:
        horizon = int(horizon)
        score_col = f"score_el_{horizon}m"
        method_col = f"metodo_{horizon}m"
        method = str(out[method_col].iloc[0]) if method_col in out.columns and len(out) else ""
        meta_h = horizon_models.get(horizon, {})
        trained = meta_h.get("status") == "trained"
        counts_valid = not ("fallback_percentil" in method or "sin_modelo" in method)

        if trained:
            calibrated = str(meta_h.get("calibration_status", "")).startswith("probabilidad")
            reliability = (
                "alta_probabilidad_calibrada" if calibrated
                else "media_score_no_calibrado_conteo_aproximado"
            )
        elif counts_valid:
            reliability = "media_baja_extrapolado_de_otro_horizonte"
        else:
            reliability = "baja_solo_ranking_sin_conteo"

        p = pd.to_numeric(out.get(score_col), errors="coerce").clip(0, 1).fillna(0.0)
        if "el_confirmado_previo" in out.columns:
            # El modelo estima riesgo de PRIMER evento: quien ya tiene EL confirmada no cuenta
            # en los casos esperados futuros (queda marcado en la tabla de scoring).
            p = p.where(out["el_confirmado_previo"].eq(0), 0.0)
        if counts_valid:
            e_total, lo_total, hi_total = _expected_with_ci(p)
        else:
            e_total, lo_total, hi_total = np.nan, np.nan, np.nan

        horizon_rows.append({
            "horizon_months": horizon,
            "poblacion_activa_puntuada": int(len(out)),
            "casos_esperados": e_total,
            "ci95_bajo": lo_total,
            "ci95_alto": hi_total,
            "riesgo_medio": float(p.mean()) if len(p) else np.nan,
            "metodo_scoring": method,
            "confiabilidad": reliability,
        })

        # --- probabilidades por patologia y trabajador ---
        # Fase 1: senal cruda por grupo (modelo especifico si existe Y supera al azar; si no,
        # descomposicion score global x mezcla historica).
        raw_probs: Dict[str, pd.Series] = {}
        methods: Dict[str, str] = {}
        for g in groups:
            meta_g = (pathology_models.get(g) or {}).get(horizon)
            use_model = bool(meta_g and meta_g.get("status") == "trained")
            if use_model:
                # Compuerta de calidad: un modelo cuyo PR-AUC no supera la prevalencia (azar)
                # no aporta senal; se prefiere la descomposicion transparente.
                bm = meta_g.get("best_metrics", {}) or {}
                pr_auc_g = bm.get("pr_auc")
                prev_g = bm.get("prevalence")
                if pd.notna(pr_auc_g) and pd.notna(prev_g) and float(pr_auc_g) <= float(prev_g):
                    use_model = False
            if use_model:
                raw = meta_g["model"].predict_proba(out[meta_g["feature_cols"]])[:, 1]
                raw_probs[g] = pd.Series(raw, index=out.index).clip(0, 1)
                methods[g] = "modelo_especifico_por_patologia"
            else:
                if mix_by_cargo is not None and "cargo" in out.columns and g in mix_by_cargo.columns:
                    mix_vec = out["cargo"].map(mix_by_cargo[g]).fillna(float(global_mix.get(g, 0.0)))
                else:
                    mix_vec = pd.Series(float(global_mix.get(g, 0.0)), index=out.index)
                raw_probs[g] = (p * mix_vec).clip(0, 1)
                methods[g] = "descomposicion_score_global_x_mezcla_historica"

        # Fase 2: reconciliacion proporcional con el modelo global. Los modelos por patologia
        # (a menudo entrenados con pocos positivos y sin calibrar) aportan la MEZCLA relativa;
        # el modelo global, mejor validado, aporta el NIVEL. Se reescala cada persona para que
        # la suma de sus probabilidades por patologia iguale su probabilidad global, de modo
        # que los conteos por patologia siempre sumen el conteo total y las participaciones
        # nunca superen el 100%.
        raw_frame = pd.DataFrame(raw_probs)
        row_sum = raw_frame.sum(axis=1)
        scale = (p / row_sum.replace(0, np.nan)).clip(upper=1e6)
        recon = raw_frame.mul(scale, axis=0)
        for g in groups:
            recon[g] = recon[g].fillna(p * float(global_mix.get(g, 0.0)))
        pat_probs: Dict[str, pd.Series] = {}
        for g in groups:
            p_g = recon[g].clip(0, 1)
            pat_probs[g] = p_g
            out[f"score_el_{horizon}m_pat_{g}"] = p_g
            if counts_valid:
                e_g, lo_g, hi_g = _expected_with_ci(p_g)
            else:
                e_g, lo_g, hi_g = np.nan, np.nan, np.nan
            pat_rows.append({
                "horizon_months": horizon,
                "patologia": g,
                "patologia_label": PATHOLOGY_GROUPS[g]["label"],
                "casos_esperados": e_g,
                "ci95_bajo": lo_g,
                "ci95_alto": hi_g,
                "participacion_pct": (100.0 * e_g / e_total) if counts_valid and e_total and e_total > 0 else np.nan,
                "metodo": methods[g] + "+reconciliado_con_total_global",
                "confiabilidad": reliability,
            })

        pat_frame = pd.DataFrame(pat_probs)
        dominant = pat_frame.idxmax(axis=1)
        # Un empate total por fila (mezcla uniforme por falta de historia clasificable) no es
        # senal: idxmax resolveria arbitrariamente hacia la primera patologia de la taxonomia.
        _no_signal = (pat_frame.max(axis=1) <= 0) | (pat_frame.nunique(axis=1) <= 1)
        out[f"patologia_mas_probable_{horizon}m"] = dominant.where(~_no_signal, "sin_senal")

        # --- agregaciones por cargo y por centro ---
        for dim, sink in (("cargo", cargo_rows), ("centro", centro_rows)):
            if dim not in out.columns:
                continue
            for value, idx in out.groupby(dim, dropna=False).groups.items():
                if pd.isna(value):
                    continue
                sub_p = p.loc[idx]
                if counts_valid:
                    e_u, lo_u, hi_u = _expected_with_ci(sub_p)
                else:
                    e_u, lo_u, hi_u = np.nan, np.nan, np.nan
                unit_expected = {g: float(pat_probs[g].loc[idx].sum()) for g in groups}
                top = sorted(unit_expected.items(), key=lambda kv: kv[1], reverse=True)[:3]
                if counts_valid:
                    top_txt = "; ".join(
                        f"{PATHOLOGY_GROUPS[g]['label']} ({v:.2f})" for g, v in top if v > 0
                    ) or "sin_senal"
                else:
                    # Sin conteo proyectable (fallback de percentiles): solo etiquetas
                    # ordenadas, sin cifras que parezcan casos esperados.
                    top_txt = "; ".join(
                        PATHOLOGY_GROUPS[g]["label"] for g, v in top if v > 0
                    ) or "sin_senal"
                sink.append({
                    "horizon_months": horizon,
                    dim: value,
                    "n_trabajadores": int(len(idx)),
                    "casos_esperados": e_u,
                    "ci95_bajo": lo_u,
                    "ci95_alto": hi_u,
                    "riesgo_medio": float(sub_p.mean()) if counts_valid else np.nan,
                    "patologias_principales": top_txt,
                    "confiabilidad": reliability,
                })

    tables = {
        "por_horizonte": pd.DataFrame(horizon_rows),
        "por_patologia": pd.DataFrame(pat_rows).sort_values(
            ["horizon_months", "casos_esperados"], ascending=[True, False], na_position="last"
        ),
        "por_cargo": pd.DataFrame(cargo_rows).sort_values(
            ["horizon_months", "casos_esperados", "riesgo_medio"],
            ascending=[True, False, False], na_position="last",
        ) if cargo_rows else pd.DataFrame(),
        "por_centro": pd.DataFrame(centro_rows).sort_values(
            ["horizon_months", "casos_esperados", "riesgo_medio"],
            ascending=[True, False, False], na_position="last",
        ) if centro_rows else pd.DataFrame(),
    }
    log.add(
        "prediccion", "OK",
        "Proyecciones construidas por horizonte/patología"
        + ("/cargo" if cargo_rows else "") + ("/centro" if centro_rows else ""),
    )
    return out, tables


def _md_cell(value: Any) -> str:
    """Escapa el separador de columnas de markdown en valores de datos (cargos/centros)."""
...
def generate_predictive_report(
    ...
    """Escribe informe_predictivo.md: resumen legible por humanos y por asistentes de IA."""
    lines: List[str] = []
    add = lines.append
    latest = scored["periodo"].max() if "periodo" in scored.columns and not scored.empty else None

    add(f"# Informe predictivo de enfermedad laboral — {cfg.COMPANY_NAME}")
    add("")
    add(f"Generado: {datetime.now():%Y-%m-%d %H:%M}. Periodo de referencia del scoring: "
        f"{latest:%Y-%m}. Población activa puntuada: {len(scored)}." if latest is not None
        else f"Generado: {datetime.now():%Y-%m-%d %H:%M}.")
    add("")
    add("## Cómo leer este informe")
    add("")
    add("- **Casos esperados** = suma de probabilidades individuales de la población activa; es un valor esperado condicional que asume que los patrones históricos se mantienen. El intervalo (IC 95%) refleja solo la incertidumbre estadística del conteo, no la del modelo.")
    add("- **Confiabilidad**: `alta_probabilidad_calibrada` permite leer los números como conteos esperados; `media_*` como magnitudes aproximadas; `baja_solo_ranking_sin_conteo` significa que ese horizonte solo ordena el riesgo (no se proyectan conteos).")
    add("- Las patologías sin modelo propio usan la descomposición *score global × mezcla histórica de patologías* (suavizada por cargo): asumen que la mezcla de patologías del pasado se mantiene.")
    add("- Todas las probabilidades por patología se **reconcilian proporcionalmente** con el modelo global (mejor validado): los modelos por patología aportan la mezcla relativa y el global el nivel, de modo que la suma por patología siempre iguala el total esperado.")
    add("- Este informe NO diagnostica, no califica origen, no determina PCL y no debe usarse para decisiones disciplinarias. Es un instrumento de priorización preventiva del SG-SST.")
    add("")

    add("## Calidad de los modelos por horizonte")
    add("")
    add("| Horizonte | Estado | Modelo | PR-AUC | AUC | Calibración | Selección | Split |")
    add("|---|---|---|---|---|---|---|---|")
    for h in cfg.HORIZONS_MONTHS:
        meta = horizon_models.get(int(h), {})
        best = meta.get("best_metrics", {}) or {}
        add(
            f"| {int(h)} m | {meta.get('status', 'sin_info')} | {meta.get('model_name', '-')} "
            f"| {_fmt_num(best.get('pr_auc'), 3)} | {_fmt_num(best.get('auc_roc'), 3)} "
            f"| {meta.get('calibration_status', '-')} | {meta.get('selection_confidence', '-')} "
            f"| {meta.get('split_method', '-')} |"
        )
    add("")

    por_h = tables.get("por_horizonte", pd.DataFrame())
    por_pat = tables.get("por_patologia", pd.DataFrame())
    por_cargo = tables.get("por_cargo", pd.DataFrame())
    por_centro = tables.get("por_centro", pd.DataFrame())

    for h in cfg.HORIZONS_MONTHS:
        h = int(h)
        add(f"## Horizonte {h} meses")
        add("")
        row = por_h[por_h["horizon_months"].eq(h)]
        if not row.empty:
            r = row.iloc[0]
            if pd.notna(r["casos_esperados"]):
                add(f"- **Casos esperados**: {_fmt_num(r['casos_esperados'])} "
                    f"(IC 95%: {_fmt_num(r['ci95_bajo'])}–{_fmt_num(r['ci95_alto'])}) "
                    f"sobre {int(r['poblacion_activa_puntuada'])} trabajadores activos. "
                    f"Confiabilidad: `{r['confiabilidad']}`.")
            else:
                add(f"- Este horizonte no tiene modelo con conteo proyectable "
                    f"(confiabilidad `{r['confiabilidad']}`); use el ranking de riesgo, no conteos.")
        add("")
        sub_pat = por_pat[por_pat["horizon_months"].eq(h)].head(6)
        if not sub_pat.empty:
            add("**Patologías más probables:**")
            add("")
            add("| Patología | Casos esperados | IC 95% | % del total | Método |")
            add("|---|---|---|---|---|")
            for _, r in sub_pat.iterrows():
                add(f"| {r['patologia_label']} | {_fmt_num(r['casos_esperados'])} "
                    f"| {_fmt_num(r['ci95_bajo'])}–{_fmt_num(r['ci95_alto'])} "
                    f"| {_fmt_num(r['participacion_pct'])}% | {r['metodo']} |")
            add("")
        sub_cargo = por_cargo[por_cargo["horizon_months"].eq(h)].head(10) if not por_cargo.empty else pd.DataFrame()
        if not sub_cargo.empty:
            add("**Cargos con mayor carga esperada:**")
            add("")
            add("| Cargo | Trabajadores | Casos esperados | Riesgo medio | Patologías principales |")
            add("|---|---|---|---|---|")
            for _, r in sub_cargo.iterrows():
                add(f"| {_md_cell(r['cargo'])} | {int(r['n_trabajadores'])} | {_fmt_num(r['casos_esperados'])} "
                    f"| {_fmt_num(100 * r['riesgo_medio'])}% | {_md_cell(r['patologias_principales'])} |")
            add("")
        sub_centro = por_centro[por_centro["horizon_months"].eq(h)].head(10) if not por_centro.empty else pd.DataFrame()
        if not sub_centro.empty:
            add("**Centros de trabajo con mayor carga esperada:**")
            add("")
            add("| Centro / sede / ciudad | Trabajadores | Casos esperados | Riesgo medio | Patologías principales |")
            add("|---|---|---|---|---|")
            for _, r in sub_centro.iterrows():
                add(f"| {_md_cell(r['centro'])} | {int(r['n_trabajadores'])} | {_fmt_num(r['casos_esperados'])} "
                    f"| {_fmt_num(100 * r['riesgo_medio'])}% | {_md_cell(r['patologias_principales'])} |")
            add("")

    add("## Uso previsto y prohibiciones")
    add("")
    add("- Uso: prevención, vigilancia epidemiológica, priorización de validaciones e inversión en controles del SG-SST.")
    add("- Prohibido: diagnóstico individual, calificación de origen, determinación de PCL, decisiones de contratación, sanción, traslado o retiro.")
    add("")
    add("## Nota para análisis con IA")
    add("")
    add("Este informe es autocontenido: puede pegarse en un asistente de IA junto con la pregunta que se quiera responder (p. ej., \"¿en qué cargos y centros debo priorizar controles ergonómicos el próximo año?\"). Pida siempre que la respuesta respete las banderas de confiabilidad y las prohibiciones de uso indicadas arriba.")
    add("")
...
# =============================================================================
# CELDA 15 — 12. EXPLICABILIDAD, EQUIDAD Y DRIFT
# =============================================================================
# %%
# =============================================================================
...
def fairness_diagnostics(scored: pd.DataFrame, dimension: str, min_n: int = 30) -> pd.DataFrame:
    if dimension not in scored.columns:
        return pd.DataFrame()
    if "el_confirmado_previo" in scored.columns:
        # El diagnostico de equidad evalua el trato del MODELO; los casos ya confirmados no
        # estan modelados y distorsionarian las tasas por grupo.
        scored = scored[scored["el_confirmado_previo"].eq(0)]
    rows: List[Dict[str, Any]] = []
...
def psi(expected: pd.Series, actual: pd.Series, bins: int = 10, min_samples_per_bin: int = 20) -> float:
    ...
    # Bins adaptativos: con muestras chicas se reduce la cantidad de bins en vez de devolver
    # NaN (un piso fijo de bins*min dejaria el monitoreo de drift silenciosamente inoperante
    # en organizaciones medianas, cuyo snapshot mensual rara vez supera unos cientos de filas).
    n_min = int(min(len(exp), len(act)))
    if n_min < 2 * min_samples_per_bin:
        return np.nan
    bins = max(2, min(bins, n_min // min_samples_per_bin))
...
def drift_report(
    meta: Dict[str, Any], current: pd.DataFrame, cfg: ProjectConfig, log: Optional["RunLog"] = None
) -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame()
    table = pd.DataFrame(rows).sort_values("psi", ascending=False, na_position="last")
    if log is not None and table["psi"].isna().all():
        log.add(
            "drift", "WARN",
            f"Horizonte {meta.get('horizon')}: PSI no evaluable en ninguna variable "
            f"(muestra del snapshot actual: {len(current)} filas, insuficiente).",
        )
    return table
...
# =============================================================================
# CELDA 16 — 13. PLAN DE ACCION, MODEL CARD Y EXPORTACION
# =============================================================================
# %%
# =============================================================================
...
# =============================================================================
# CELDA 17 — 14. ORQUESTACION PRINCIPAL
# =============================================================================
# %%
# =============================================================================
...
def run_pipeline(cfg: ProjectConfig = CONFIG) -> Dict[str, Any]:
    ...
    # Modelos especificos por patologia: solo donde hay masa critica; el resto de patologias
    # se proyecta por descomposicion (score global x mezcla historica) en project_predictions.
    pathology_models: Dict[str, Dict[int, Dict[str, Any]]] = {}
    if cfg.ENABLE_PATHOLOGY_MODELS:
        pat_target_cols = [c for c in panel_target.columns if re.match(r"target_el_\d+m_pat_", c)]
        pathology_groups_present = sorted({re.sub(r"target_el_\d+m_pat_", "", c) for c in pat_target_cols})
        for group in pathology_groups_present:
            pathology_models[group] = {}
            for horizon in cfg.HORIZONS_MONTHS:
                horizon = int(horizon)
                tcol = f"target_el_{horizon}m_pat_{group}"
                ecol = f"target_eligible_{horizon}m_pat_{group}"
                if tcol not in panel_target.columns or ecol not in panel_target.columns:
                    continue
                pat_positives = int(panel_target.loc[panel_target[ecol].eq(1), tcol].sum())
                if pat_positives < cfg.PATHOLOGY_MIN_POSITIVES:
                    log.add(
                        "modelo_patologia", "INFO",
                        f"{horizon}m/{group}: {pat_positives} positivos, por debajo del minimo "
                        f"{cfg.PATHOLOGY_MIN_POSITIVES}; se proyectara por descomposicion de mezcla historica.",
                    )
                    continue
                meta_pat = train_one_horizon(
                    panel_target, horizon, key, cfg, dirs, log,
                    target_col=tcol, eligibility_col=ecol, tag=f"_pat_{group}",
                )
                if meta_pat.get("status") == "trained":
                    pathology_models[group][horizon] = meta_pat

    scored, fallback_meta = score_current_population(panel_target, horizon_models, data, key, cfg, log)

    # Proyecciones predictivas: patologias, cargos y centros por horizonte.
    el_events_classified = pd.DataFrame()
    if not target_event_source.empty:
        _classified = classify_el_records(target_event_source, cfg)
        el_events_classified = _classified[_classified["target_event"].eq(1)].copy()
        if not el_events_classified.empty:
            el_events_classified["pathology_group"] = classify_pathology_group(el_events_classified)

    scored, prediction_tables = project_predictions(
        scored, horizon_models, pathology_models, el_events_classified, roster, key, cfg, log,
    )
    save_df(prediction_tables["por_horizonte"], dirs["tables"] / "12_prediccion_por_horizonte.csv")
    save_df(prediction_tables["por_patologia"], dirs["tables"] / "13_prediccion_por_patologia.csv")
    save_df(prediction_tables["por_cargo"], dirs["tables"] / "14_prediccion_por_cargo.csv")
    save_df(prediction_tables["por_centro"], dirs["tables"] / "15_prediccion_por_centro.csv")
    if cfg.GENERATE_PREDICTIVE_REPORT:
        report_path = dirs["root"] / "informe_predictivo.md"
        generate_predictive_report(prediction_tables, horizon_models, scored, cfg, report_path)
        log.add("prediccion", "OK", f"Informe predictivo generado: {report_path}")
    ...
    for dimension in ["cargo", "area", "centro"]:
        table = aggregate_priority(scored, dimension, min_n=cfg.MIN_SAMPLE_FOR_CI)
    ...
    for horizon, meta in horizon_models.items():
        ...
        drift = drift_report(meta, scored, cfg, log=log)
        if not drift.empty:
    ...
    for dimension in ["sex", "age_band", "centro", "area"]:
        ...
        fairness = fairness_diagnostics(scored, dimension, min_n=cfg.MIN_SAMPLE_FOR_CI)
    ...
    pathology_status_rows: List[Dict[str, Any]] = []
    for group, metas in pathology_models.items():
        for horizon in cfg.HORIZONS_MONTHS:
            meta_pat = metas.get(int(horizon))
            pathology_status_rows.append({
                "patologia": group,
                "horizon_months": int(horizon),
                "status": meta_pat.get("status") if meta_pat else "descomposicion_mezcla_historica",
                "model_name": meta_pat.get("model_name") if meta_pat else None,
                "calibration_status": meta_pat.get("calibration_status") if meta_pat else None,
                "selection_confidence": meta_pat.get("selection_confidence") if meta_pat else None,
            })
    pathology_status = pd.DataFrame(pathology_status_rows)
    save_df(pathology_status, dirs["tables"] / "06_estado_modelos_patologia.csv")
    ...
    model_card = {
        ...
        "pathology_models": pathology_status_rows,
        "interpretation_rules": [
            "Las proyecciones de casos esperados son valores esperados condicionales que asumen continuidad de los patrones históricos; interprete siempre junto a su bandera de confiabilidad e IC 95%.",
            "Las patologías sin modelo propio se proyectan por descomposición (score global × mezcla histórica): asumen que la mezcla de patologías del pasado se mantiene.",
        ...
        "known_limitations": [
            ...
            "Purga anti-fuga: las filas de entrenamiento cuya ventana de outcome cruza el "
            "corte temporal se descartan; en horizontes largos esto reduce el entrenamiento "
            "disponible y puede dejar horizontes sin modelo propio (usaran extrapolacion o "
            "fallback, siempre marcados en selection_confidence/metodo).",
            "Las ventanas de outcome de meses consecutivos de una misma persona se solapan por "
            "construccion; la validacion cruzada agrupada por entidad y la purga del corte "
            "mitigan la fuga, pero las metricas de CV pueden conservar un leve optimismo "
            "residual por solapamiento entre personas distintas.",
    ...
    summary_tables = {
        ...
        "prediccion_horizonte": prediction_tables.get("por_horizonte", pd.DataFrame()),
        "prediccion_patologia": prediction_tables.get("por_patologia", pd.DataFrame()),
        "prediccion_cargo": prediction_tables.get("por_cargo", pd.DataFrame()),
        "prediccion_centro": prediction_tables.get("por_centro", pd.DataFrame()),
    ...
    return {
        ...
        "predictions": prediction_tables,
        "pathology_models": pathology_models,
...
# =============================================================================
# CELDA 18 — 15. EJEMPLO DE CONFIGURACION EN COLAB
# =============================================================================
# %%
# =============================================================================
...
if __name__ == "__main__":
    ...
    CONFIG.HORIZONS_MONTHS = (12, 24, 60)
