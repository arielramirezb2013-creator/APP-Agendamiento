...
function wordsToNumbers(text) {
  ...
  while (i < toks.length) {
    ...
    let j = i; const run = [];
    /* Una cifra en dígitos («600») se une a la corrida solo si le sigue un multiplicador («mil»), para no fusionar direcciones o teléfonos. */
    while (j < toks.length && (isNumW(toks[j]) || (toks[j] === "y" && j + 1 < toks.length && NUMW[toks[j + 1]] !== undefined && run.length) || (/^\d+$/.test(toks[j]) && j + 1 < toks.length && MULTW[toks[j + 1]] !== undefined))) { if (toks[j] !== "y") run.push(toks[j]); j++; }
...
const F = { ans: {}, meta: {}, pending: [], cur: null, tries: 0, phase: "idle", ronda: 0, sec: null, run: 0, cont: false, infer: null, modo: "guion", gdone: {}, gbuf: "", gtimer: null, flushMs: 250 };
